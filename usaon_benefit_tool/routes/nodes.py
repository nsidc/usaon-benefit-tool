...
from usaon_benefit_tool.models.tables import Node, NodeSubtypeOther, NodeSubtypeSocietalBenefitArea, AssessmentNode
...
def export():
    ...
    for node in nodes:
        ...
        subtype_sba = NodeSubtypeSocietalBenefitArea.query.filter_by(node_id=node.id).first()
    ...
    return Response(
        ...
        mimetype='text/csv',
        headers={
            'Content-Disposition': f'attachment; filename=object-library-{date.today()}.csv'
        }
    )
